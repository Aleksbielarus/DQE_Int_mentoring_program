import connection
import Credentials
import pandas as pd


def test_hr_empl_duplicates_by_key_checks():
    cursor, conn = connection.mssql_connect(Credentials.server,
                                            Credentials.user,
                                            Credentials.password,
                                            Credentials.database,
                                            Credentials.port)
    cursor.execute(f'SELECT employee_id, count(*) '
                   f'FROM [hr].[employees]'
                   f'GROUP BY employee_id '
                   f'HAVING count(*) > 1')
    results = len(pd.DataFrame(cursor.fetchall()).index)
    assert results == 0


def test_hr_empl_salary_validation():
    cursor, conn = connection.mssql_connect(Credentials.server,
                                            Credentials.user,
                                            Credentials.password,
                                            Credentials.database,
                                            Credentials.port)
    cursor.execute(f' SELECT employee_id, employees.job_id, salary, min_salary, max_salary'
                   f' FROM [hr].[employees]'
                   f'    LEFT JOIN [hr].[jobs] on jobs.job_id = employees.job_id'
                   f' WHERE salary < min_salary or salary > max_salary')
    results = len(pd.DataFrame(cursor.fetchall()).index)
    assert results == 0


def test_hr_empl_email_validity():
    cursor, conn = connection.mssql_connect(Credentials.server,
                                            Credentials.user,
                                            Credentials.password,
                                            Credentials.database,
                                            Credentials.port)
    cursor.execute(f" SELECT employee_id, email"
                   f" FROM hr.employees"
                   f" WHERE email NOT LIKE '%@%';")
    results = len(pd.DataFrame(cursor.fetchall()).index)
    assert results == 0


def test_hr_empl_manager_id_consistancy():
    cursor, conn = connection.mssql_connect(Credentials.server,
                                            Credentials.user,
                                            Credentials.password,
                                            Credentials.database,
                                            Credentials.port)
    cursor.execute(
                    f" SELECT em.employee_id, em.manager_id"
                    f" FROM [hr].[employees] em"
                    f" WHERE manager_id NOT IN (SELECT employee_id FROM [hr].[employees])")
    results = len(pd.DataFrame(cursor.fetchall()).index)
    assert results == 0


def test_hr_location_postal_code_validity():
    cursor, conn = connection.mssql_connect(Credentials.server,
                                            Credentials.user,
                                            Credentials.password,
                                            Credentials.database,
                                            Credentials.port)
    cursor.execute(f" SELECT location_id, postal_code"
                   f" FROM [hr].[locations]"
                   f" WHERE LEN(postal_code) != 5")
    results = len(pd.DataFrame(cursor.fetchall()).index)
    assert results == 0


def test_hr_empl_hire_date_ccompleteness():
    cursor, conn = connection.mssql_connect(Credentials.server,
                                            Credentials.user,
                                            Credentials.password,
                                            Credentials.database,
                                            Credentials.port)
    cursor.execute(f" SELECT * "
                   f"FROM[hr].[employees]"
                   f"WHERE hire_date IS NULL;")
    results = len(pd.DataFrame(cursor.fetchall()).index)
    assert results == 0

